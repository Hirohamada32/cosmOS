...
void map_page(uint32_t *table1, uint32_t vaddr, paddr_t paddr, uint32_t flag){
	...
	table0[vpn0] = ((paddr / PAGE_SIZE) << 10) | flag | PAGE_V;
}
...
void yield(void){
	...
	for(int i = 0; i < PROC_MAX; i++){
		...
		if(proc->state == PROC_RUNNABLE && proc->pid > 0){ // runnable & initialised
			next = proc;
			break;
		}
	}
...
void virtio_blk_init(void){
	if(virtio_reg_read32(VIRTIO_REG_MAGIC) != 0x74726976)
		PANIC("virtio: invalid magic number");
...
void kernel_main(void) {
	...
	//printf("alloc_page test: paddr0=%x\t paddr1=%x\n", paddr0, paddr1);
